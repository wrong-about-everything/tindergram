...
use TG\Domain\Gender\Impure\FromPure as FromPure;
use TG\Domain\Gender\Impure\NonSuccessful as NonSuccessful;
use TG\Domain\Gender\Pure\FromInteger;
use TG\Domain\Gender\Pure\NonExistent;
use TG\Infrastructure\ImpureInteractions\ImpureValue;

class BotUserPreferredGender extends Gender
{
    private $botUser;
    private $concrete;
    ...
    private function doConcrete(): Gender
    {
        ...
        if (!$this->botUser->value()->pure()->isPresent() || is_null($this->botUser->value()->pure()->raw()['preferred_gender'])) {
            return new FromPure(new NonExistent());
        }

        return new FromPure(new FromInteger($this->botUser->value()->pure()->raw()['preferred_gender']));
    }
}